<script>
    $('#example').on('click', '.dropdown-item', function() {
        ...
        const url = '{{ route('administrator.master.jenis-kegiatan.show', ':id') }}'.replace(':id', dataId);
    ...
    function handleShow(data) {
        $('#detail_kode_jenis').text(data.kode_jenis || 'N/A');
        $('#detail_nama_jenis').text(data.nama_jenis || 'N/A');
        $('#detail_tahun').text(data.tahun || 'N/A');
        $('#detail_keterangan').text(data.keterangan || 'N/A');
        $('#detail_status').text(data.status || 'N/A');
        $('#modalDetail').modal('show');
    }

    function handleEdit(data) {
        $('#editForm').attr('data-id', data.id_jenis_kegiatan);
