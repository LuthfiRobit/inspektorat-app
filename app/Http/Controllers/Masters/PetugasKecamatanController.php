...
class PetugasKecamatanController extends Controller
{
    protected $responseService;
    protected $transactionService;
    protected $logActivityService;
    protected $userAccountService;
    protected $scope = 'kecamatan';
    ...
    /**
     * Display the index view for Petugas Kecamatan.
     */
    public function index()
    {
        $this->logActivityService->log('Accessed the index view for Petugas Kecamatan');
        return view('administration.masters.petugas.kecamatan.index');
    }

    /**
     * Retrieve and return the list of Petugas Kecamatan for DataTables.
     */
    public function list(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('aksi', function ($item) {
                ...
                $hasShow = $user->hasPermissionTo('master.petugas.kecamatan.view');
                $hasEdit = $user->hasPermissionTo('master.petugas.kecamatan.edit');
                ...
                $hasDelete = $user->hasPermissionTo('master.petugas.kecamatan.delete');
                if ($hasDelete) {
    ...
    /**
     * Display the create view for Petugas Kecamatan.
     */
    public function create()
    {
        $this->logActivityService->log('Accessed the create view for Petugas Kecamatan');
        return view('administration.masters.petugas.kecamatan.create');
    }

    /**
     * Store a new Petugas Kecamatan record in the database.
     */
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            $this->logActivityService->log('Validation failed during Petugas Kecamatan store', 'Errors: ' . json_encode($validator->errors()));
        ...
        $this->logActivityService->log('Stored new Petugas Kecamatan', 'Data: ' . json_encode($request->all()));
    ...
    /**
     * Display the details of a specific Petugas Kecamatan by ID.
     */
    public function show($id)
    {
        ...
        $this->logActivityService->log('Viewed Petugas Kecamatan detail', 'ID: ' . $id);
        return $this->responseService->success($petugas);
    }

    /**
     * Display the edit view for Petugas Kecamatan.
     */
    public function edit($id)
    {
        $this->logActivityService->log('Accessed the edit view for Petugas Kecamatan');
        $petugas = Petugas::whereNotNull('kecamatan_id')
            ->whereNull('desa_id')
            ->find($id);
        $kecamatans = Kecamatan::where('status', 'active')->get();
        return view('administration.masters.petugas.kecamatan.edit', compact('petugas', 'kecamatans'));
    }

    /**
     * Update an existing Petugas Kecamatan record.
     */
    public function update(Request $request, $id)
    {
        $petugas = Petugas::whereNotNull('petugas.kecamatan_id')->whereNull('petugas.desa_id')
            ->find($id);

        if (!$petugas) {
            $this->logActivityService->log('Petugas Kecamatan not found for update', 'ID: ' . $id);
            return $this->responseService->error('Data not found', ResponseService::STATUS_NOT_FOUND);
        }
        $validationRules = [
            'kecamatan_id' => ['required', 'exists:kecamatan,id_kecamatan'],
        ...
        if ($validator->fails()) {
            $this->logActivityService->log(
                'Validation failed during Petugas Kecamatan update',
    ...
    public function resetPassword($id)
    {
        ...
        $this->logActivityService->log('Reset Password Petugas Kecamatan', 'ID: ' . $id . ' NIP: ' . $petugas->nip);
    ...
    /**
     * Remove the specified Petugas Kecamatan from storage (Soft Delete).
     */
    public function destroy($id)
    {
        $petugas = Petugas::whereNotNull('kecamatan_id')->whereNull('desa_id')->find($id);

        if (!$petugas) {
            $this->logActivityService->log('Petugas Kecamatan not found for delete', 'ID: ' . $id);
            return $this->responseService->error('Data not found', ResponseService::STATUS_NOT_FOUND);
        }
        ...
        try {
            ...
            if ($userId) {
                ...
                // Cek jika dia developer, cegah hapus
                if ($user && $user->isDeveloper()) {
                    \Illuminate\Support\Facades\DB::rollBack();
                    return $this->responseService->error('Akun Developer tidak boleh dihapus.', 403);
                }
                if ($user && !$user->isDeveloper()) {
                    $user->delete();
                }
            ...
            $this->logActivityService->log('Deleted Petugas Kecamatan (Soft Delete)', 'ID: ' . $id);
