    ...
    public function list(Request $request)
    {
        ...
        return DataTables::of($data)
            ->addColumn('aksi', function ($row) {
                ...
                // Tombol Lihat Revisi - hanya untuk status 'revision'
                $btnRevisi = '';
                if ($row['status'] === 'revision') {
                    $btnRevisi = '<a class="dropdown-item" href="javascript:void(0);" data-action="action_show_revision" 
                                    data-id-laporan="' . $row['id_laporan'] . '">
                                <i class="fas fa-history me-2"></i>Lihat Revisi
                            </a>';
                }
            ...
            ->addColumn('review_info', function ($row) {
                ...
                if ($row['status'] === 'submitted') {
                    $info = '<span class="badge badge-primary">Butuh Review</span>';
                } elseif ($row['status'] === 'revision') {
                    $info = '<span class="badge badge-warning">Menunggu Revisi</span>';
                }
